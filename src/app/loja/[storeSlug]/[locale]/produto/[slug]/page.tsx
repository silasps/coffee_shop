...
export default async function StoreProductPage({
  ...
  params: Promise<{ storeSlug: string; locale: string; slug: string }>;
}) {
  const { storeSlug, locale, slug } = await params;
  ...
  const [store, product] = await Promise.all([
    ...
    getProductBySlug(typedLocale, slug, storeSlug),
  ]);
      ...
      <section className="site-shell mt-6 grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
        ...
        <div className="glass-panel rounded-[34px] p-7 md:p-10">
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            ...
            />
            <Link
              href={buildStorePath(storeSlug, typedLocale, `/menu/${product.area}`)}
